...
internal protocol FloatOrDouble {
    static var cblas_gemm:
        (
            _ __Order: CBLAS_ORDER, _ __TransA: CBLAS_TRANSPOSE, _ __TransB: CBLAS_TRANSPOSE,
            _ __M: Int32, _ __N: Int32, _ __K: Int32, _ __alpha: Self, _ __A: UnsafePointer<Self>?,
            _ __lda: Int32, _ __B: UnsafePointer<Self>?, _ __ldb: Int32, _ __beta: Self,
            _ __C: UnsafeMutablePointer<Self>?, _ __ldc: Int32
        ) -> Void
    { get }
}
...
internal func fastMultiplyMatrices<M>(lhs: M, rhs: M) -> M
where M: FastInitializableMatrix, M.ElementType: FloatOrDouble {
    var vals = [M.ElementType](repeating: M.ElementType.zero, count: M.size * M.size)
    M.ElementType.cblas_gemm(
        CblasRowMajor, CblasNoTrans, CblasNoTrans, Int32(M.size), Int32(M.size), Int32(M.size),
        M.ElementType.one, lhs.elements, Int32(M.size), rhs.elements, Int32(M.size),
        M.ElementType.zero, &vals, Int32(M.size))
    return M(exactElements: vals)
}

internal func fastMultiply<M, V>(matrix: M, vector: V) throws -> V
where
    ...
    M.ElementType: FloatOrDouble, M.ElementType == V.ComponentType
{
    ...
    M.ElementType.cblas_gemm(
        CblasRowMajor, CblasNoTrans, CblasNoTrans, Int32(matrix.dimensions.rows), 1,
        Int32(matrix.dimensions.columns), M.ElementType.one, matrix.elements,
        Int32(matrix.dimensions.columns), vector.components, Int32(vector.dimensions.columns),
        M.ElementType.zero, &output, Int32(vector.dimensions.columns))
