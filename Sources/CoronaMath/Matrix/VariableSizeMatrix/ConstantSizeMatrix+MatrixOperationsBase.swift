...
extension ConstantSizeMatrix where Self: MatrixOperationsBase {
    ...
    public func add<M>(by matrix: M) throws -> Self
    where
        M: MatrixBase,
    ...
    public func subtract<M>(by matrix: M) throws -> Self
    where
        M: MatrixBase,
        ElementType == M.ElementType
    {
...
public func + <M>(lhs: M, rhs: M.ElementType) -> M
where M: ConstantSizeMatrix, M: MatrixOperationsBase {
    return M(elements: lhs.elements.map { $0 + rhs })
}

public func + <M>(lhs: M.ElementType, rhs: M) -> M
where M: ConstantSizeMatrix, M: MatrixOperationsBase {
