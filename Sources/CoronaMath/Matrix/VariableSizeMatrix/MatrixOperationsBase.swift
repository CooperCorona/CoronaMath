...
public protocol MatrixOperationsBase: MatrixBase {
    ...
    associatedtype MatrixAdditionType: MatrixBase
    where MatrixAdditionType.ElementType == Self.ElementType
    ///The type returned by multiplying two matrices.
    associatedtype MatrixMultiplicationType: MatrixBase
    where MatrixMultiplicationType.ElementType == Self.ElementType
    ///The type returned by transposing this matrix.
    associatedtype MatrixTransposeType: MatrixBase
    where MatrixTransposeType.ElementType == Self.ElementType
    ...
    func add<M>(by matrix: M) throws -> MatrixAdditionType
    where
        M: MatrixBase,
    ...
    func subtract<M>(by matrix: M) throws -> MatrixAdditionType
    where
        M: MatrixBase,
        ElementType == M.ElementType
    ...
    func multiply<M>(by matrix: M) throws -> MatrixMultiplicationType
    where
        M: MatrixBase,
        ElementType == M.ElementType
