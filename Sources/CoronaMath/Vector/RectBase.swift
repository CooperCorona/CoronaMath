...
public struct RectBase<VectorType>: ConstantSizeVector, CustomStringConvertible
where VectorType: DiscreteNumber {
    ...
    // MARK: - Static Properties
        ...
        public private(set) var components: [VectorType] = [VectorType](
            repeating: VectorType.zero, count: RectBase.numberOfComponents)
    #else
        ///The number of components in a `RectBase` instance.
        ...
        ///The values of the vector.
        public private(set) var components: [VectorType] = [VectorType](
            repeating: VectorType.zero, count: RectBase.staticNumberOfComponents)
    #endif

    ///The unit rectangle in the `VectorType` space. Has origin zero and size one.
    public static var unitRect: RectBase<VectorType> {
        return RectBase(
            x: VectorType.zero, y: VectorType.zero, width: VectorType.one, height: VectorType.one)
    }
...
extension RectBase: ContinuousVector, ConstantSizeContinuousVector
...
extension RectBase where VectorType: Comparable {
    ...
    public func intersects(rect: RectBase<VectorType>) -> Bool {
        return
            !(self.minX > rect.maxX || self.maxX < rect.minX || self.minY > rect.maxY
            || self.maxY < rect.minY)
    }
...
private enum RectBaseCodingKeys: CodingKey {
    case x
    case y
    case width
    case height
