...
public struct SizeBase<VectorType>: ConstantSizeVector, CustomStringConvertible
where VectorType: DiscreteNumber {
    ...
    public typealias ComponentType = VectorType
        ...
        public private(set) var components = [VectorType](
            repeating: VectorType.zero, count: SizeBase.numberOfComponents)
    #else
        ///The number of components in a `SizeBase` instance.
        ...
        ///The values of this vector.
        public private(set) var components = [VectorType](
            repeating: VectorType.zero, count: SizeBase.staticNumberOfComponents)
    #endif
...
extension SizeBase where VectorType: DiscreteNumber {
    ...
    public static var unitWidth: SizeBase<VectorType> {
        return SizeBase<VectorType>(components: [VectorType.one, VectorType.zero])
    }
    ///The unit vector in the height direction.
    public static var unitHeight: SizeBase<VectorType> {
        return SizeBase<VectorType>(components: [VectorType.zero, VectorType.one])
    }
}
...
extension SizeBase where VectorType == Float {
    ...
    public var center: PointBase<Float> {
        return PointBase(x: self.width / 2.0, y: self.height / 2.0)
    }
...
extension SizeBase where VectorType == Double {
    ...
    ///The center of the rectangle with origin `{0, 0}` and size `self`.
    public var center: PointBase<Double> {
        return PointBase(x: self.width / 2.0, y: self.height / 2.0)
    }

}

private enum SizeBaseCodingKeys: CodingKey {
    case width
    case height
}
...
public func + <VectorType>(lhs: SizeBase<VectorType>, rhs: PointBase<VectorType>) -> SizeBase<
    VectorType
> {
...
public func / <VectorType: ContinuousNumber>(lhs: SizeBase<VectorType>, rhs: PointBase<VectorType>)
    -> SizeBase<VectorType>
{
    return SizeBase(width: lhs.width / rhs.x, height: lhs.height / rhs.y)
}
